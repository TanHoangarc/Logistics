...
function DuplicateApp() {
  ...
  const userName = "Hoang Dan";
  
  const addActivity = (content: string, type: 'submit' | 'approve' | 'settle' | 'other') => {
    setActivities(prev => {
      const newActivities = [
        { id: Math.random().toString(36).substr(2, 9), timestamp: new Date().toISOString(), content, type },
        ...prev
      ];
      return newActivities.slice(0, 20);
    });
  ...
  const handleAddKimberryJob = (j: Omit<KimberryJob, 'id'>) => {
    setKimberryJobs([...kimberryJobs, { id: Math.random().toString(36).substr(2, 9), ...j }]);
  };
  ...
  const handleAddBankTransaction = (t: Omit<BankTransaction, 'id'>) => {
    setBankTransactions([...bankTransactions, { id: Math.random().toString(36).substr(2, 9), ...t }]);
  };
  ...
  const handleSaveExpense = (data: Partial<Expense>) => {
    ...
    setExpenses([newExpense, ...expenses]);
    addActivity(`${userName} đã gửi yêu cầu quyết toán ${new Intl.NumberFormat('vi-VN').format(newExpense.amount)} vnđ cho hoá đơn ${newExpense.type}`, 'submit');
    setIsModalOpen(false);
  };
  ...
  const handleApprove = (id: string) => {
    setExpenses(expenses.map(ex => {
      if (ex.id === id) {
        addActivity(`A Nguyễn đã phê duyệt yêu cầu quyết toán ${ex.type} của ${ex.submitter}`, 'approve');
        return { ...ex, isApproved: true };
      }
      return ex;
    }));
  ...
  const handleSettle = (id: string) => {
    setExpenses(expenses.map(ex => {
      if (ex.id === id) {
        addActivity(`B Phạm đã quyết toán hoá đơn ${ex.type} cho ${ex.submitter} với số tiền ${new Intl.NumberFormat('vi-VN').format(ex.amount)} vnđ`, 'settle');
        return { ...ex, isSettled: true };
      }
      return ex;
    }));
  ...
  const handleAddCashTransaction = (t: Omit<CashTransaction, 'id' | 'voucherNumber'>) => {
    let voucherNumber = '';
    if (t.type === 'receipt') {
      voucherNumber = `PT${String(receiptCount + 1).padStart(5, '0')}`;
      setReceiptCount(prev => prev + 1);
    } else {
      voucherNumber = `PC${String(paymentCount + 1).padStart(5, '0')}`;
      setPaymentCount(prev => prev + 1);
    }
  ...
  const currentUserEmployee = employees.find(e => e.fullName === userName || e.fullName.includes(userName) || e.name === userName);
  const currentUserAvatar = currentUserEmployee?.avatarUrl;

  return (
    <Router>
      <Layout role={role} setRole={setRole} currentUserName={userName} currentUserAvatar={currentUserAvatar}>
        <Routes>
          <Route path="/" element={<WelcomeView onOpenModal={() => setIsModalOpen(true)} userName={userName} activities={activities} />} />
          <Route 
            path="/phe-duyet" 
            element={
              <ApprovalView 
          ...
          />
          <Route path="/bao-cao" element={<ViewPlaceholder title="Báo cáo tổng quan" />} />
          
          {/* Finance Routes */}
          <Route 
            path="/tai-chinh/nhat-ky" 
            element={
              <CashBookView 
                transactions={cashTransactions}
                initialBalance={initialBalance}
          ...
          />
          <Route path="/tai-chinh/thanh-toan" element={<ViewPlaceholder title="Thanh toán" />} />
          <Route path="/tai-chinh/doanh-thu" element={<ViewPlaceholder title="Doanh thu" />} />
          <Route path="/tai-chinh/ket-qua" element={<ViewPlaceholder title="Kết quả kinh doanh" />} />
          <Route path="/tai-chinh/luu-chuyen" element={<ViewPlaceholder title="WCA" />} />
          <Route path="/tai-chinh/doi-soat" element={<ViewPlaceholder title="Đối soát COD" />} />
          <Route path="/tai-chinh/thue" element={<ViewPlaceholder title="Thuế VAT" />} />
          <Route 
            path="/tai-chinh/kimberry" 
            element={
              <KimberryView
                jobs={kimberryJobs}
                onAddJob={handleAddKimberryJob}
          ...
          <Route path="/nhan-vien/cham-cong" element={<AttendanceView employees={employees} attendanceRecords={attendanceRecords} onUpdateAttendance={handleUpdateAttendance} />} />
          <Route path="/nhan-vien/nghi-phep" element={<LeaveManagementView employees={employees} attendanceRecords={attendanceRecords} onUpdateAttendance={handleUpdateAttendance} />} />
          <Route path="/nhan-vien/luong" element={<ViewPlaceholder title="Lương" />} />

          {/* HR Administration Routes */}
          <Route 
            path="/hanh-chinh/nhan-vien" 
            element={
              <EmployeeManagementView 
                employees={employees} 
                onAdd={() => { setEditEmp(undefined); setIsEmpModalOpen(true); }}
          ...
          />
          <Route 
            path="/hanh-chinh/chi-phi" 
            element={
              <ExpenseCategoryManagementView 
                categories={expenseCategories} 
                onAdd={() => { setEditCat(undefined); setIsCatModalOpen(true); }}
                onEdit={(c) => { setEditCat(c); setIsCatModalOpen(true); }}
                onDelete={handleDeleteCategory}
              />
            } 
          />

          {/* Business module (Công việc) */}
          <Route path="/cong-viec/pricing/freights" element={<FreightsView />} />
          <Route path="/cong-viec/pricing/local-charges" element={<LocalChargesView />} />
          <Route path="/cong-viec/pricing/services" element={<ServicesView />} />
          <Route path="/cong-viec/pricing/custom-charge" element={<CustomChargeView />} />
          <Route path="/cong-viec/pricing/sales-request" element={<SalesRequestView />} />

          <Route path="/cong-viec/sales/quotes" element={<ViewPlaceholder title="Quotes" />} />
          <Route path="/cong-viec/sales/customer-request" element={<ViewPlaceholder title="Customer Request" />} />

          <Route path="/cong-viec/setting" element={<ViewPlaceholder title="Setting" />} />
        </Routes>
